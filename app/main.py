...
from app.routers.munim import router as munim_router
...
# Arthsetu Merchant Help Integration
from app.munim.routers import voice as munim_voice_router
from app.munim.routers import dashboard as munim_dashboard_router
from app.munim.routers import udhari as munim_udhari_router
from app.munim.routers import whatsapp as munim_whatsapp_router
from app.munim.services.socketio_manager import get_socketio_manager
from app.munim.data.mock_store import seed_demo_data, load_mock_data
from app.munim.config import get_munim_config

munim_config = get_munim_config()
...
def on_startup() -> None:
    ...
    # Arthsetu Merchant Help Initialization
    try:
        load_mock_data()
        if munim_config.SEED_DEMO_DATA:
            seed_demo_data("demo_merchant")
        print("✅ Arthsetu Merchant Help initialized with mock data")
    except Exception as exc:
        print(f"⚠️  Arthsetu Merchant Help initialization warning: {exc}")
...
app.include_router(munim_router, prefix="/api", tags=["munim"])
app.include_router(cashflow_router, tags=["cashflow"])

# Arthsetu Merchant Help Routers
app.include_router(munim_voice_router.router, tags=["munim-voice"])
app.include_router(munim_dashboard_router.router, tags=["munim-dashboard"])
app.include_router(munim_udhari_router.router, tags=["munim-udhari"])
app.include_router(munim_whatsapp_router.router, tags=["munim-whatsapp"])

# Health check for Arthsetu Merchant Help
@app.get("/api/munim/health")
async def munim_health():
    return {
        "status": "healthy",
        "munim_version": "1.0.0",
        "mock_mode": munim_config.MOCK_MODE,
        "services": {
            "groq": munim_config.is_configured("groq"),
            "openai": munim_config.is_configured("openai"),
            "twilio": munim_config.is_configured("twilio"),
            "tavily": munim_config.is_configured("tavily"),
            "sarvam": munim_config.is_configured("sarvam")
        }
    }
